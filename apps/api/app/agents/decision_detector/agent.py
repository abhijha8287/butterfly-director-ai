...
class DecisionDetectorAgent(BaseAgent[DecisionDetectorRequest, DecisionList]):
    """Third agent in the pipeline, built on the same reference shape as the
    Story and Character Architects: a ChatOpenAI model pointed at DashScope, a
    ...
    def _build_messages(
        ...
        developer_prompt = load_prompt(PROMPTS_DIR, PROMPT_VERSION, "developer.txt").format(
            min_candidates=self._settings.decision_branch_candidates_min,
            max_candidates=self._settings.decision_branch_candidates_max,
        )
    ...
    async def run(self, request: DecisionDetectorRequest) -> AgentRunResult[DecisionList]:
        llm = self._build_llm()
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            try:
                decisions = self._parser.parse(last_raw_text)
                validate_against_story_bible(decisions, request.story_bible, self._settings)
            except _RETRYABLE_PARSE_ERRORS as exc:
                last_error = exc
                logger.warning(
                    "decision_detector_attempt_failed",
                    attempt=attempt,
                    error=str(exc)[:300],
                )
                repair_note = (
        ...
        raise AgentOutputInvalidError(
            f"Decision Detector failed to produce a valid DecisionList after "
