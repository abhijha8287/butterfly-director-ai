...
class TimelineGeneratorAgent(BaseAgent[TimelineGeneratorRequest, TimelineGenerationResult]):
    """Fourth agent in the pipeline, built on the same reference shape as the
    other three: a ChatOpenAI model pointed at DashScope, a
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._parser = PydanticOutputParser(pydantic_object=TimelineGenerationResult)

    def _build_llm(self) -> ChatOpenAI:
        return get_llm(self._settings, temperature=0.8)

    def _build_messages(
        self, request: TimelineGeneratorRequest, repair_note: str | None
    ) -> list[BaseMessage]:
        ...
        decision_json = request.decision.model_dump_json(indent=2)
        user_content = (
            f"{developer_prompt}\n\n"
            f"StoryBible:\n{bible_json}\n\n"
            f"Decision:\n{decision_json}\n\n"
    ...
    async def run(self, request: TimelineGeneratorRequest) -> AgentRunResult[TimelineGenerationResult]:
        llm = self._build_llm()
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            try:
                ...
                validate_against_decision(result, request.decision)
            except _RETRYABLE_PARSE_ERRORS as exc:
                last_error = exc
                logger.warning(
                    "timeline_generator_attempt_failed",
                    attempt=attempt,
                    error=str(exc)[:300],
                )
                repair_note = (
            ...
            logger.info("timeline_generator_succeeded", attempt=attempt, latency_ms=latency_ms)
        ...
        raise AgentOutputInvalidError(
            f"Timeline Generator failed to produce a valid TimelineGenerationResult after "
