...
class CharacterArchitectAgent(BaseAgent[CharacterRequest, CharacterRoster]):
    """Second agent in the pipeline, built on the Story Architect's reference
    shape: a ChatOpenAI model pointed at DashScope, a PydanticOutputParser for
    the output contract, and a self-driven repair loop that feeds validation
    errors back to the model instead of giving up.
    """
    ...
    async def run(self, request: CharacterRequest) -> AgentRunResult[CharacterRoster]:
        llm = self._build_llm()
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            try:
                roster = self._parser.parse(last_raw_text)
                validate_against_story_bible(roster, request.story_bible)
            except _RETRYABLE_PARSE_ERRORS as exc:
                last_error = exc
                logger.warning(
                    "character_architect_attempt_failed",
                    attempt=attempt,
                    error=str(exc)[:300],
                )
                repair_note = (
        ...
        raise AgentOutputInvalidError(
            f"Character Architect failed to produce a valid CharacterRoster after "
