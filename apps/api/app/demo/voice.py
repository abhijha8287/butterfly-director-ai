...
against the live DashScope API and a real Postgres database, and prints the
extracted dialogue lines and their synthesis outcome.

Usage:
    python -m app.demo.voice
"""
...
async def main() -> None:
    async with async_session_factory() as session:
        project = await ProjectRepository(session).create(
            title="Voice Demo",
            premise="Demo project created by python -m app.demo.voice",
        )
            ...
            json.dumps(
                {
                    "lines": [
                        {
                            "shot": line.shot_number,
                            "character": line.character_name,
                            "line": line.line_text,
                            "delivery_note": line.delivery_note,
                            "audio_path": line.asset.oss_key,
                        }
                        for line in voice_response.lines
                    ],
                    "failed": [
                        {
                            "shot": f.shot_number,
                        ...
                        for f in voice_response.failed_lines
                    ],
                },
                indent=2,
            )
        ...
        print("\n--- generation metadata ---")
        print(
            f"model={voice_response.model} prompt_version={voice_response.prompt_version} "
            f"latency_ms={voice_response.latency_ms} attempts={voice_response.attempts} "
            f"prompt_tokens={voice_response.prompt_tokens} "
            f"completion_tokens={voice_response.completion_tokens}"
        )
