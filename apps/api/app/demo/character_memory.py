...
Memory - against the live DashScope API and a real Postgres database, and
prints each character's resolved state_diff + drift judgment for the first
generated branch.

Usage:
    python -m app.demo.character_memory
"""
...
async def main() -> None:
    async with async_session_factory() as session:
        project = await ProjectRepository(session).create(
            title="Character Memory Demo",
            premise="Demo project created by python -m app.demo.character_memory",
        )
        ...
        memory_response = await CharacterMemoryService(session).generate(branch.id)

        print(json.dumps([s.model_dump(mode="json") for s in memory_response.states], indent=2))
        print("\n--- drift summary ---")
        for state in memory_response.states:
            flag = f" <-- {state.drift_warning}" if state.drift_severity != "none" else ""
            print(f"{state.character_name}: drift={state.drift_severity}{flag}")
        print("\n--- generation metadata ---")
        print(
            f"model={memory_response.model} prompt_version={memory_response.prompt_version} "
            f"latency_ms={memory_response.latency_ms} attempts={memory_response.attempts} "
            f"prompt_tokens={memory_response.prompt_tokens} "
            f"completion_tokens={memory_response.completion_tokens}"
        )
