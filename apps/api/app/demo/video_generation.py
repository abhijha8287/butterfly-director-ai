...
Memory -> Storyboard -> Prompt Director -> Video Generation - against the
live DashScope API and a real Postgres database, and prints the rendered (or
failed) outcome for each shot.

Usage:
    python -m app.demo.video_generation
"""
...
async def main() -> None:
    async with async_session_factory() as session:
        project = await ProjectRepository(session).create(
            ...
            premise="Demo project created by python -m app.demo.video_generation",
        )
            ...
            json.dumps(
                {
                    "rendered": [
                        {
                            "shot": r.shot_number,
                            "video_url": r.asset.oss_key,
                            "asset_id": str(r.asset.id),
                        }
                        for r in video_response.rendered
                    ],
                    "failed": [
                        {"shot": f.shot_number, "attempts": f.attempts, "error": f.error}
                        for f in video_response.failed_shots
                    ],
                },
                indent=2,
            )
        ...
        print("\n--- generation metadata ---")
        print(
            f"model={video_response.model} prompt_version={video_response.prompt_version} "
            f"latency_ms={video_response.latency_ms} attempts={video_response.attempts} "
            f"movie_id={video_response.movie_id} job_id={video_response.job_id}"
        )
