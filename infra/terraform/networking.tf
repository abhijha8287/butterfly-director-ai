...
# --- NAT gateway (ECS tasks need outbound internet to call Gemini/DashScope) ---
...
resource "aws_security_group" "api" {
  ...
  tags = merge(local.common_tags, { Name = "${local.prefix}-sg-api" })
}

resource "aws_security_group" "worker" {
  name        = "${local.prefix}-worker"
  description = "ECS worker and beat tasks (no inbound)"
  vpc_id      = aws_vpc.main.id
  ...
  tags = merge(local.common_tags, { Name = "${local.prefix}-sg-worker" })
}

resource "aws_security_group" "rds" {
  name        = "${local.prefix}-rds"
  description = "RDS: allow Postgres from ECS tasks"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "efs" {
  ...
  tags = merge(local.common_tags, { Name = "${local.prefix}-sg-efs" })
}
